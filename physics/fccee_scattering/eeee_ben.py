...
def ee_ee(C, par, E, cthmin, cthmax):
    ...
    return np.real(sigma_tot)

def ee_ee_obs(wc_obj, par, E):
    scale = E 
    C = wc_obj.get_wcxf(sector='all', scale=scale, par=par,
                        eft='SMEFT', basis='Warsaw')
    selection_efficiency = 0.98
    return selection_efficiency * ee_ee(C, par, E, -0.9, 0.9)


def AFB_ee_obs(wc_obj, par, E):
    scale = E 
    C = wc_obj.get_wcxf(sector='all', scale=scale, par=par,
                        eft='SMEFT', basis='Warsaw')
    AFBee = (ee_ee(C, par, E, 0, 0.9) - ee_ee(C,par, E, -0.9, 0)) / ee_ee(C, par, E, -0.9, 0.9)
    return AFBee
...
_obs.set_description(r"Cross section of $" + _process_tex + r"$ at energy $E$")
_obs.tex = r"$\sigma(" + _process_tex + r")$"
_obs.add_taxonomy(_process_taxonomy)
print("Prediction for ", _obs_name, "added")

#_process_tex = r"AFB e^+e^- \to e^+e^-"
#_process_taxonomy = r'Process :: $e^+e^-$ scattering :: AFB $e^+e^-\to e^+e^-$ :: $' + _process_tex + r"$"
#_obs_name = "AFB(ee->ee)(high_E)"
#_obs = Observable(_obs_name)
#_obs.arguments = ['E']
#Prediction(_obs_name, AFB_ee_obs)
#_obs.set_description(r"AFB of $" + _process_tex + r"$ at energy $E$")
#_obs.tex = r"$AFB(" + _process_tex + r")$"
#_obs.add_taxonomy(_process_taxonomy)
#
#print("Prediction for ", _obs_name, "added")
